...
| An injectable `ApplicationClock` (`now()`, `currentLocalDate()`) is threaded through nearly every mutation, and `createFixedClock` already exists. | `src/shared/lib/local-date/clock.ts:95` | The server can construct a per-request clock. No new abstraction is needed for FR-009. |
...
## Decision 5: The request clock — client date, server instant

**Decision**: Every request carries the client's current local date in an
`X-Orbit-Local-Date` header. A Fastify hook validates it and builds a per-request
`ApplicationClock` where `currentLocalDate()` returns the client-supplied date and `now()`
returns the server's UTC instant.

**Rationale**: FR-009's intent is that no date-sensitive behavior may depend on the server's
timezone. `currentLocalDate()` is the only value with timezone meaning — it drives closure
eligibility (FR-039), recurrence effective boundaries (FR-019), and the automatic habit
boundary miss (FR-020) — so it must come from the client, exactly as today. `Instant` is by
construction a UTC timestamp with no timezone component, so taking it from the server is
already timezone-independent, and it is the better source: instants are audit values
(`createdAt`, `occurredAt`, `updatedAt`), and sourcing them server-side keeps them
monotonic and immune to a skewed client clock.

**This satisfies FR-009 rather than deviating from it** — the requirement's stated purpose is
"rather than the server's own timezone or system date," and a UTC instant carries neither
timezone nor local-date meaning. Event *ordering* does not depend on instants in any case:
`task_events` carry an explicit monotonic sequence, and habit outcome events carry an
`ordinal`. Recorded here explicitly because it refines how FR-009 applies to `now()`.

**Alternatives considered**:

- *Client supplies the instant too*: rejected. A wrong client clock would corrupt audit
  timestamps permanently, and it buys nothing since instants have no timezone semantics.
- *Server derives the local date from its own timezone*: rejected. This is the exact failure
  FR-009 exists to prevent; a container running UTC would roll days over at the wrong moment
...
| Who owns audit instants? | Server UTC clock (Decision 5). |
